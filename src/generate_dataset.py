...
generate_dataset.py
-------------------
Creates a realistic synthetic Telco Customer Churn dataset.

Run this once:  python -m src.generate_dataset
...
def internet_addon(service, yes_p=0.28):
    return np.where(
        service == "No",
        "No internet service",
...
monthly_charges = np.round(
    20
    + 10 * (internet_service == "Fiber optic").astype(int)
...
OUTPUT_FILE = DATA_DIR / "telco_churn.csv"

df.to_csv(OUTPUT_FILE, index=False)
print(f"Dataset saved -> {OUTPUT_FILE} ({len(df):,} rows x {len(df.columns)} cols)")
